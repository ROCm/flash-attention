...
fmha_fwd_splitkv_args get_ck_fmha_varlen_fwd_splitkv_args(bool has_lse,
                                                          ...
                                                          const at::Tensor k,
                                                          const at::Tensor v,
                                                          const at::Tensor seqstart_q,
                                                          const at::Tensor seqstart_k,
                                                          ...
                                                          at::Tensor out_acc)
{
    ...
    // lse: (nheads, total_q)
    // randval: (nheads, total_q, max_seqlen_k)
    // lse_acc: (split, nheads, total_q)
    // o_acc: (split, nheads, seqlen_q, d)
    ...
    args.seqstart_q_ptr = seqstart_q.data_ptr();
    args.seqstart_k_ptr = seqstart_k.data_ptr();
               ...
               c10::optional<at::Generator> gen_)
{
    ...
    auto softmax_lse_accum = torch::empty({num_splits, num_heads, total_q}, opts.dtype(at::kFloat));
    auto out_accum = torch::empty({num_splits, num_heads, total_q, head_size}, opts.dtype(at::kFloat));
    ...
    if (max_seqlen_k > 0) {
        ...
        if (paged_kv) {
            auto traits =
                get_ck_fmha_fwd_varlen_splitkv_traits(
                    mask,
                    q_dtype_str,
                    head_size,
                    has_lse,
            ...
            fmha_fwd_splitkv(traits, args, stream_config);
        }
        else {
            auto traits =
