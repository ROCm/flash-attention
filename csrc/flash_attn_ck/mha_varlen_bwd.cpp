...
void log_varlen_bwd_config(const char* func_name, const fmha_bwd_traits& fmha_traits, const fmha_bwd_args& fmha_args){
  ...
  if (flash_attn_log_config) {
    ...
    //std::cout<<"dropout_offset: "<<std::get<1>(std::get<std::pair<const void*, const void*>>(fmha_args.drop_seed_offset))<<std::endl;Add commentMore actions
  }
}
               ...
               c10::optional<at::Tensor> &rng_state)
{
    ...
    if (max_seqlen_q > 0) {
        ...
        log_varlen_bwd_config(__FUNCTION__, traits, args);
