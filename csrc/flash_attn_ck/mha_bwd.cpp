                                   ...
                                   std::pair<uint64_t*, uint64_t*> drop_seed_offset)
{
    ...
    // dq_acc: (num_heads, split, batch_size, seqlen_q, hdim)
    ck_tile::index_t split_stride_dq_acc = dq_acc.stride(1);
    ck_tile::long_index_t batch_stride_dq_acc = dq_acc.stride(2);
        ...
        std::optional<at::Tensor> &rng_state_)
{
    ...
    if (!deterministic) {
        dq_accum = torch::zeros({num_heads, 1, batch_size, seqlen_q, head_size}, opts.dtype(at::kFloat));
    } else {
        const ck_tile::index_t kN0 = head_size <= 128 ? 128 : 64;
        const ck_tile::index_t nsplits = ck_tile::integer_divide_ceil(seqlen_k, kN0);
        dq_accum = torch::zeros({num_heads, nsplits, batch_size, seqlen_q, head_size}, opts.dtype(at::kFloat));
    }
