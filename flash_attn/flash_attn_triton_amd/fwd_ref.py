...
def attention_forward_core_ref_impl(
    ...
    if is_paged:
        ...
        assert (
            nheads_q % nheads_k == 0
        ), f"nheads_q ({nheads_q}) must be divisible by nheads_k ({nheads_k})"
        ...
        if False:  # Debug disabled
            print(
                f"L_q: {L_q}, L_k: {L_k}, nheads_q: {nheads_q}, nheads_k: {nheads_k}, group_size: {group_size}, head_dim: {head_dim}"
            )
            print(
                f"block_table contents: {block_table if block_table is not None else 'None'}"
            )

        # Initialize attention scores
        attention_scores = torch.zeros(
            (nheads_q, L_q, L_k), dtype=torch.float32, device=q.device
        )
        ...
        for kv_pos in range(L_k):
            ...
            k_vec = k[physical_block, within_block_idx, :, :].to(
                torch.float32
            )  # [nheads_k, head_dim]
    ...
    if alibi_slopes is not None:
        if cache_seqlens is not None:
            ...
            row_idx = torch.arange(L_q, device=q.device, dtype=torch.float32).unsqueeze(
                1
            )
            col_idx = torch.arange(L_k, device=q.device, dtype=torch.float32).unsqueeze(
                0
            )
            ...
            if DEBUG_CORE:
                print(
                    f"Decode ALiBi: cache_seqlens={cache_seqlens}, L_q={L_q}, L_k={L_k}"
                )
    ...
    if mask_applied:
        ...
        sum_exp_scores = torch.where(
            sum_exp_scores == 0, torch.ones_like(sum_exp_scores), sum_exp_scores
        )
    if DEBUG_CORE:
        print("sum_exp_scores:", sum_exp_scores, sum_exp_scores.shape)
    ...
    if is_paged:
        ...
        assert (
            nheads_q % nheads_v == 0
        ), f"nheads_q ({nheads_q}) must be divisible by nheads_v ({nheads_v})"
        ...
        for kv_pos in range(L_k):
            ...
            v_vec = v_paged[physical_block, within_block_idx, :, :].to(
                torch.float32
            )  # [nheads_v, head_dim]
...
def attention_vanilla_forward_pytorch_ref_impl(
    ...
    if group_size != 1:
        ...
        sd_mask = sd_mask.reshape(
            batch_size, nheads_k, group_size, seq_len_q, seq_len_k
        )
...
def attention_varlen_forward_pytorch_ref_impl(
    ...
    softmax_lse = torch.zeros(
        (nheads_q, total_L_q), dtype=torch.float32, device=q.device
    )
    sd_mask = torch.zeros(
        (batch_size, nheads_q, max_seqlen_q, max_seqlen_k),
        dtype=torch.float32,
        device=q.device,
    )
...
def attention_prefill_forward_ref_impl(
    ...
    if (rotary_cos is not None) or (rotary_sin is not None):
        raise NotImplementedError(
            "Rotary embeddings not implemented in reference prefill path."
        )
    # compute reference
    if layout == "thd":
...
def attention_decode_forward_ref_impl(
    ...
    if (rotary_cos is not None) or (rotary_sin is not None):
        raise NotImplementedError(
            "Rotary embeddings not implemented in reference decode path."
        )
    ...
    if k_new is not None and v_new is not None:
        if is_paged:
            ...
            _, seq_len_new, _, _ = (
                k_new.shape
            )  # shape is [batch, seq_len, nheads, head_dim] for bshd layout
            ...
            for b in range(batch_size):
                ...
                for pos_offset in range(seq_len_new):
                    ...
                    k_cache[physical_block, within_block_idx, :, :] = k_new[
                        b, pos_offset, :, :
                    ]
                    v_cache[physical_block, within_block_idx, :, :] = v_new[
                        b, pos_offset, :, :
                    ]
        else:
            _, seq_len_new, _, _ = (
    ...
    ), f"Head dimensions must match: {head_dim}, {head_dim_k}, {head_dim_v}"
    ...
    softmax_lse = torch.zeros(
        (batch_size, nheads_q, seq_len_q), dtype=torch.float32, device=q.device
    )
